...
/**
 * Class that is responsible for sending requests to external APIs and deserializing the response
 * received from those APIs.
 */
public class CensusAPIUtilities {
  private CensusAPIUtilities() {}

  /**
   * Method that sends a request for state codes, deserializes the response and stores them in a
   * stateMap.
   *
  ...
  public static Map<String, State> deserializeStateCodes() throws IOException {
    ...
    List<List<String>> statesData =
        adapter.fromJson(
            new Buffer().readFrom(clientConnection.getInputStream())); // reads Jsondata to a List
  ...
  /**
   * Method that sends a request to the CensusAPI, deserializes the response, and stores it in a
   * Map.
   *
  ...
  /**
   * Method that sends a request for broadband information given a state code and county code,
   * deserializes it, and returns the output.
   *
