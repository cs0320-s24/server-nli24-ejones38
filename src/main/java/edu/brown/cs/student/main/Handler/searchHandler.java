...
public class searchHandler implements Route {
  ...
  /**
   *The constructor of searchHandler. Takes in a CSVWrapper which contains useful data about the contents of a CSV file
  ...
  /**
   * Method handles incoming viewcsv requests. Checks and responds if a csv has not been loaded or if the proper params
   * were not included in the search request. Else searches the loaded CSV file for the specified value
   * (optionally at the specified header/index). Converts the search data into a JSON file and outputs it along with the
   * appropriate server response. Method also checks to make sure index requests are valid else returns an error JSON
   * @param request - the incoming user request
   * @param response - the outgoing response back to the user
   * @return - a JSON string representing the rows of the CSV file with the specified value along with appropriate
   * server response
   */
  @Override
