...
public class testHandlers {
  ...
  public void testViewWithoutLoad() throws IOException {
    ...
    assertEquals(200, clientConnection1.getResponseCode());
    Map<String, Object> response =
        this.adapter.fromJson(new Buffer().readFrom(clientConnection1.getInputStream()));
    assertEquals("error", response.get("result"));
  }
  ...
  public void testColNameSuccess() throws IOException {
    ...
    HttpURLConnection clientConnection1 =
        tryRequest("searchcsv?value=Providence&columnName=City/Town");
    assertEquals(200, clientConnection1.getResponseCode());
    Map<String, Object> response =
        this.adapter.fromJson(new Buffer().readFrom(clientConnection1.getInputStream()));
    assertEquals("success", response.get("result"));
  ...
  public void testNoCounty() throws IOException {
    HttpURLConnection clientConnection =
        tryRequest("broadband?state=Washington&county=VladniraDepena");
    assertEquals(200, clientConnection.getResponseCode());
    Map<String, Object> response =
        this.adapter.fromJson(new Buffer().readFrom(clientConnection.getInputStream()));
    assertEquals("error", response.get("result"));
